 ...
 * @v2 **removeAll** : Demande de suppression de toutes les figures d'une icône
 *
 * @version v2
 */
@Injectable()
export class FiguresService {
  ...
  async update(
    ...
    if (figure !== null) {
      ...
      if (newOrder && newOrder !== order) {
        const switchFigure = await Figure.findOneBy({
          icon: { id: icon.id },
          order: newOrder,
        });
   ...
   * @version v2
   */
  async remove(icon: Icon, order: number): Promise<Figure | null> {
    if (order !== icon.figures.length) {
      await Promise.all([
        ...icon.figures.map(async (fig) => {
          if (fig.order > order) {
            fig.order -= 1;
    ...
    const figure = await Figure.findOneBy({
      icon: { id: icon.id },
      order: icon.figures.length,
    });
  ...
  /**
   * Demande de suppression de toutes les figures d'une icône
   *
   * @param icon icône dans laquelle on souhaite supprimer les figures
   *
   * @returns Les figure supprimées
   *
   * @version v2
   */
  async removeAll(icon: Icon): Promise<Figure[]>  {
    await Promise.all([...icon.figures.map(async (fig) => await fig.remove())]);
